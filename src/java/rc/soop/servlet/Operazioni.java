...
import static rc.soop.action.Constant.patterndatesql;
...
//import static rc.soop.util.Utility.sendMailHtml;
...
public class Operazioni extends HttpServlet {
    ...
    protected void registraUtente(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String bandorif = Constant.bando;
...
//                                                        boolean es = sendMailHtml(dest, "Registrazione", text, null);
                                                    try {
                                                        boolean es = SendMailJet.sendMail(Constant.titlepro, dest, new String[]{}, text, Constant.titlepro + " - Registrazione");
                                                        if (es) {
                                                            trackingAction("service", "Registrato nuovo Utente: " + username);
                                                            ok = true;
                                                        }
                                                    } catch (Exception e) {
                                                        trackingAction("service", "Errore registrazione: " + ExceptionUtils.getStackTrace(e));
                                                    }
                                                }
                                            }
                                        }
                    ...
                    redirect(request, response, "home.jsp?esito=KO4&bando=" + bandorif);
                }
            } catch (Exception ex) {
                trackingAction("service", "Errore registrazione: " + ExceptionUtils.getStackTrace(ex));
            }
        }
        if (ok) {
...
//                }
        ...
        if (att) {
            if (!pres) {
                ...
                if (es) {
                    ...
                    try {
                        SendMailJet.sendMail(Constant.titlepro, dest, new String[]{}, text, Constant.titlepro + " - Conferma invio domanda di partecipazione");
    ...
    protected void annulladomanda(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (es1) {
            ...
            try {
                ...
                boolean es = SendMailJet.sendMail(Constant.titlepro, dest, new String[]{}, text, Constant.titlepro + " - Cancellazione Domanda di Partecipazione");
                if (!es) {
                    msg = "1";
                }
            } catch (Exception ex) {
    ...
    protected void sendconvmail(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (SendMailJet.sendMail(Constant.titlepro, to, cc, testo, Constant.titlepro + " - Invio convenzione del Soggetto Attuatore " + var[4], new File(ActionB.getPathConvenzioni(username, tipodoc)))) {
        ...
        if (!ActionB.verificaInvioConvenzione(username)) {
            if (ActionB.setStatoInvioDocumenti(username)) {
                try {
                    ...
                    testo = testo.replaceAll("@ragsoc@", var[4]);
                    SendMailJet.sendMail(Constant.titlepro, to, new String[]{}, testo, Constant.titlepro + " - Caricamento documentazione da parte del Soggetto Attuatore " + var[4], null);
                } catch (Exception ex) {
                    Logger.getLogger(Operazioni.class.getName()).log(Level.SEVERE, null, ex);
                    ex.printStackTrace();
                }
    ...
    public void accettaDomanda(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ActionB.setStatoDomandaAccRif(username, accRif, protocollo, "---")) {
            ...
            String to[] = {var[3]};
            SendMailJet.sendMail(Constant.titlepro, to, new String[]{}, testo, Constant.titlepro + " - Accettazione domanda accreditamento");
    ...
    public void rigettaDomanda(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ActionB.setStatoDomandaAccRif(username, accRif, "", motivazione)) {
            ...
            String to[] = {var[3]};
            SendMailJet.sendMail(Constant.titlepro, to, new String[]{}, testo, Constant.titlepro + " - Rigetto presentazione domanda");
    ...
    protected void uploadMultiplo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Db_Bando dbb = new Db_Bando();
        String path = dbb.getPath("pathdoc");
        dbb.closeDB();
        ...
        if (!isMultipart) {
            ...
            try {
                ...
                ActionB.trackingAction("service", Utility.estraiEccezione(e));
            }
...
//                        }
    ...
    protected void botAreU(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ActionB.trackingAction("service", estraiEccezione(ex));
        }
